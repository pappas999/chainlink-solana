...
func (m *OCRv2TestState) ValidateFailedGauntletCommand(output string, report utils.GReport, err error, txIndex int) string {
	if err != nil {
		...
		address, errr := m.Networks.Default.(*solclient.Client).GetAddressFromSignature(sig, txIndex)
